Scriptname ExplosionScript extends Form
...
; Key to trigger the explosion
Int Property ExplosionCommand = 2  Auto ; 1 button
...
Event OnUpdate()
    ...
    If isModEnabled
        Bool isExplosionButtonPressed = Input.IsKeyPressed(ExplosionCommand);
        
        ; If the 1 button is pressed then the explosion will happen
        If isExplosionButtonPressed
            ; Kill the player and place an explosion
            Game.GetPlayer().Kill()
            Game.GetPlayer().PlaceAtMe(Explo01)  
        EndIf
